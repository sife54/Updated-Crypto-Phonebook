...
def encrypt(plaintext):                     # uses pad and the cipher key to encrypt input
    global cipher
...
class PhonebookClient(EasyFrame):
    ...
    def add(self):
        ...
        if number == "": return
        self.server.send(encrypt("ADD " + name + " " + number))
        reply = decode(self.server.recv(BUFSIZE), CODE)
        if not reply:
            self.messageBox(message="Server disconnected")
            self.disconnect()
    ...
    def update(self):
        start_book = decrypt(decode(self.server.recv(BUFSIZE)))  # decrypts the update for input to results txt field
